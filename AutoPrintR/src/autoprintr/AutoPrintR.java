    ...
    public static void main(String[] args) throws Exception {
        ...
        if (files != null) {
            for (File file : files) {
                ...
                if (file.isFile() && isPrintable(file) && !printedFiles.contains(file.getName())) {
                    try {
                        printFile(file);
                        
                        //adding the printed file name to the set
                        printedFiles.add(file.getName());
        ...
        while (true) {
            ...
            for (WatchEvent<?> event : key.pollEvents()) {
                ...
                if (kind == StandardWatchEventKinds.ENTRY_CREATE) {
                    ...
                    if (newFile.isFile() && isPrintable(newFile) && !printedFiles.contains(newFile.getName())) {
                        try {
                            printFile(newFile);
    ...
    private static void chooseFolderToWatch(){ 
        ...
        if(savedDir != null){
            ...
            while(controller == false){
                ...
                msgTxt.append("Monitoring folder: " + folderPath);
                int confirmation = JOptionPane.showConfirmDialog(null, "Are you want to use: "+ path  +" as your printer folder directory");
                if(confirmation == 0){
